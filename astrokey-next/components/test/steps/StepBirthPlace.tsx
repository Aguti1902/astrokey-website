...
export default function StepBirthPlace() {
  ...
  const searchCities = useCallback(async (q: string) => {
    ...
    try {
      // Nominatim - OpenStreetMap, gratis y sin API key
      const params = new URLSearchParams({
        ...
        limit: '12',
        featuretype: 'city',
        countrycodes: 'ad,al,at,ba,be,bg,by,ch,cy,cz,de,dk,ee,es,fi,fr,gb,gr,hr,hu,ie,is,it,li,lt,lu,lv,mc,md,me,mk,mt,nl,no,pl,pt,ro,rs,ru,se,si,sk,sm,tr,ua,va,xk',
      })
      ...
      const cities: City[] = data
        ...
        .map((item: any) => {
          ...
          const displayParts = [cityName, state, country].filter(Boolean)
          return {
            name: cityName,
      ...
      // fallback: lista estática de ciudades europeas principales
      const fallback = [
        'Madrid, España', 'Barcelona, España', 'Valencia, España', 'Sevilla, España',
        'Zaragoza, España', 'Málaga, España', 'Bilbao, España', 'Alicante, España',
        'París, Francia', 'Marsella, Francia', 'Lyon, Francia', 'Toulouse, Francia',
        ...
        'Zurich, Suiza', 'Ginebra, Suiza', 'Dublín, Irlanda',
        'Estambul, Turquía', 'Ankara, Turquía',
      ]
  ...
  const selectCity = (city: City) => {
    ...
    if (city.lat && city.lon) {
      setTestAnswer('birthPlaceCoordinates', [parseFloat(city.lon), parseFloat(city.lat)])
    }
    setShowResults(false)
    setResults([])
  }
      ...
      <div className="max-w-md mx-auto relative" ref={containerRef}>
        ...
        {!selected && query.length === 0 && (
          <p className="mt-3 text-xs text-white/25">
            Busca cualquier ciudad de Europa
          </p>
        )}
      </div>
    </div>
