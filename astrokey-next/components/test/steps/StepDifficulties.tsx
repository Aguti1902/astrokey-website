...
export default function StepDifficulties() {
  ...
  const options = [
    ...
    { value: 'amistades', Icon: Users, label: t.test.friends2 },
  ]

  const selected: string[] = Array.isArray(testAnswers.lifeDifficulties)
    ? testAnswers.lifeDifficulties
    : testAnswers.lifeDifficulties ? [testAnswers.lifeDifficulties as string] : []

  const toggle = (value: string) => {
    const updated = selected.includes(value)
      ? selected.filter((v) => v !== value)
      : [...selected, value]
    setTestAnswer('lifeDifficulties', updated)
  }

  return (
    <div className="text-center">
      <h2 className="text-2xl font-bold text-white mb-2">{t.test.difficultiesTitle}</h2>
