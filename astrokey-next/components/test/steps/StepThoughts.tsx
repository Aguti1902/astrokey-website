...
const options = [
  ...
  { value: 'amistades', label: 'Amistades', icon: '🤝' },
]
...
export default function StepThoughts() {
  ...
  return (
    <div className="text-center">
      <h2 className="text-2xl font-bold text-white mb-8">¿En qué temas has pensado últimamente?</h2>

      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-lg mx-auto">
        {options.map((opt) => (
          <button
            key={opt.value}
            onClick={() => setTestAnswer('currentThoughts', opt.value)}
            className={cn(
              'flex flex-col items-center gap-2 p-5 rounded-2xl border-2 transition-all duration-200',
              testAnswers.currentThoughts === opt.value
